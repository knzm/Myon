...
class Parse(object):
    ...
    def gettoken(self, _code, fname, current_fname):
        ...
        for i in range(len(code)):
            ...
            if self.mode == "comment":
                ...
                if self.reindex == 0 and self.token[self.reindex][2] is True:
                    print u"BlockError: ！！！突然のendブロック！！！\n" + code[i]

                idx = 0
                for j in range(self.reindex):
                    if self.token[self.reindex - j][1] == "block_name" and \
                            self.token[self.reindex-j][2] is False:
                        idx = self.reindex - j
                        break
                ...
                if self.token[self.index][3] == u"return":
                    for i in range(self.index - 1, -1, -1):
                        if self.token[i][3] == u"func" and self.token[i][2] is False:
                ...
                if self.token[self.index][5] != [[]]:
                    for i in range(len(self.token[self.index][5])):
                        self.lexer.eval.newlex([
                                u"var",
                                self.token[self.index][5][i][1],
                                str(self.depth) + u"_" + str(self.blockIndex[self.depth][1]),
                                self.token[self.index][5][i][0]])
    ...
    def getcode(self, fname, mode="extern"):
        if mode == "main":
            ...
            _pyline = u""
            for i in range(len(self.token)):
                _pyline += u" " * 4 * i nt(self.token[i][0].split("_")[0]) + self.token[i][3] + u"\n"
...
#                    print self.token
    ...
    def checkerror(self, isMain=False):
        if isMain is True and not u"Main" in [x[4] for x in self.token]:
        ...
        for i in range(len(self.token)):
            if self.token[i][1] == "block_name" and self.token[i][2] is False:
